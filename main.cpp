...
void check()
{
   if(hit==2)
	{
		MessageBox(NULL,"WELL DONE YOU WON! HAVE A GREAT DAY!!","WINNER",MB_OK);
		printf("Game Over! You Won\n");
	}
   if(hit!=2 )
	{
		MessageBox(NULL,"OOOPS YOU LOST! TRY AGAIN!! COME BACK!!!!","LOSER",MB_OK);
...
void birdb(int x,int y)
{
	...
	for(i=0.0;i<=500.0;i+=2)
	{
	    ...
	    glVertex2f(35+i,(500-y));
	    for(angle=0.0f;angle<4*360;angle+=0.01)//speed
		{
	      x2=35+sin(angle)*20;
	    ...
	    glBegin(GL_TRIANGLE_FAN);
	    glVertex2f(35+i,(500-y));
	    for(angle=0.0f;angle<4*360;angle+=10)
		{
	      x2=35+sin(angle)*20;
		  y2=(500-y)+cos(angle)*20;
		  glVertex2f(x2+i,y2);
